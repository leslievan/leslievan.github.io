...
实验分为两部分，一个让我们设计出一个不带参数的模块，功能是列出所有**内核线程**的程序名，我们称这个模块为`show_all_kernel_thread`，另一个要求是设计出一个带参数的模块，参数为某个进程的PID号，列出这个进程的父进程、子进程和兄弟进程，我们称这个模块为`show_task_family`。
...
![](<https://leslie-cloud.oss-cn-beijing.aliyuncs.com/2019/03/2019-03-26-2e80072e-1.svg>)
...
这里我们用到了名为`for_each_process(p)`的宏定义，我们可以从`include/linux/sched/signal.h`中找到这个宏定义的具体代码：
...
接下来我们编写`Makefile`文件，书上已经给出了具体的模板，这里稍作修改直接贴上：
...
同样地，我们可以根据功能写出大致的流程图：

![](https://leslie-cloud.oss-cn-beijing.aliyuncs.com/2019/03/2019-03-26-2e80072e-2.svg)
...
`pos`是数据项类型的指针，比如这里我们需要使用`task_struct`类型的数据，所以在上面的示例中，我们将`pos`声明为`task_struct *`类型.

剩下两个参数结合下图理解：

![](https://leslie-cloud.oss-cn-beijing.aliyuncs.com/2019/03/2019-03-26-2e80072e-3.svg)
...
第二个参数`head`是表头的地址，在这里就表示为`&parent.children`，第三个参数`member`指的是在链表中，`list_head *`的位置，可能会混淆的是，`task_struct`中的两个成员变量`children`和`sibling`都是`list_head *`类型，我们为什么选择`sibling`而不是`children`呢？我个人的理解是，`children`只是一个引子，只是代表一个参照物，真正进行中转的变量是`sibling`，才疏学浅，表达不太准确，有兴趣的同学可以自行查阅资料。

和上面一样，这里也直接贴出`Makefile`的代码：
...
前文提到，当模块被加载时，会运行`init`函数，在退出时，会运行`exit`函数。`printk`函数将输出打印到了日志中，我们可以使用`dmesg`命令查看系统日志，如果有遗留下的痕迹，且是正确答案，则代表测试成功。
...
选择一个既有兄弟进程又有子进程的进程（建议使用`systemd`，使用此进程作为测试，我们可以看到这个进程的PID。
